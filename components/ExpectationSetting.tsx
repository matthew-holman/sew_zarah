  ...
  return (
    <div className="mx-auto max-w-3xl rounded-3xl bg-white/10 p-10 text-center ring-1 ring-white/25">
      ...
      {pricing.body.map((paragraph, index) => (
        <p
          key={index}
          className={`${index === 0 ? "mt-4" : "mt-3"} text-base leading-relaxed text-white/70`}
        >
          {paragraph}
        </p>
      ))}
          ...
          <thead>
            <tr className="border-b border-white/10 text-white/80">
              {pricing.tableHeaders.map((header, index) => (
                <th
                  key={header}
                  className={`py-3 ${index < pricing.tableHeaders.length - 1 ? "pr-4" : ""} font-medium`}
                >
                  {header}
                </th>
              ))}
          ...
          <tbody className="text-white/70">
            {pricing.tableRows.map((row, index) => (
              <tr
                key={index}
                className={
                  index < pricing.tableRows.length - 1
      ...
      <p className="mt-6 text-sm text-white/50">{pricing.footnote}</p>
    </div>
  );
}
